...
/**
 * FloatLayout
 * It occupy entire canvas.
 * But allow a drageable box inside it.
 */
...
class BorderLayout extends Layout {
   ...
   */
  measure(layoutOwnerContainer) {
    // Measure the content size of each location
    ...
    const availableSize = layoutOwnerContainer.getSize();
    return availableSize;
  }
  /**
   * Parent tells where to start (x,y)
   */
  arrange(layoutOwnerContainer, x, y) {
    // Get available size from parent
    const { w, h } = layoutOwnerContainer.getSize();
...
/**
 * WindowLayout
 * It is a container that allow inner window to be created.
 * User must define the size of the desktop area (like a box).
 * But it also holds WindowFrame inside like a container.
 * A Window must also define its size.
 */
class WindowLayout extends Layout {
  measure() {
    return new Size(600, 600);
  }
  arrange() {
    // We always let our child (WindowFrame) to arrange itself
    for (const child of this.list) {
      child.arrange(null, 0, 0);
    }

    // But if some child try to go over boundaries, we fight back
    for (const child of this.list) {
      ...
      // Left
      if (child.position.x < border.x) {
        child.position.x = border.x;
      }
      // Top
      if (child.position.y < border.y) {
        child.position.y = border.y;
      }
      // Right
      if (child.position.x + child.size.w > border.x + border.w) {
        child.position.x = border.x + border.w - child.size.w;
      }
      // Bottom
      if (child.position.y + child.size.h > border.y + border.h) {
        child.position.y = border.y + border.h - child.size.h;
      }
    }
  }
}
